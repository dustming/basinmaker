...
def grass_raster_r_stream_extract(grass,elevation,accumulation,threshold,
                                  stream_raster,stream_vector,direction,memory):
    ...
    """      
    grass.run_command('r.stream.extract',elevation = elevation,accumulation = accumulation,threshold =threshold,stream_raster = stream_raster,
                      direction = direction,stream_vector = stream_vector, overwrite = True, memory = memory)
...
def grass_raster_r_stream_basins(grass,direction,stream,basins,memory):
    ...
    """      
    grass.run_command('r.stream.basins',direction = direction, stream = stream, basins = basins,overwrite = True,memory = memory)
...
def grass_raster_r_to_vect(grass,input,output,type,flags = '#'):
    ...
    """
    if flags == '#':   
        grass.run_command("r.to.vect", input = input, output = output, type = type, overwrite = True)
           
    else:
        grass.run_command("r.to.vect", input = input, output = output, type = type, flags = flags, overwrite = True)
...
def grass_raster_v_to_raster(grass,input,output,column,use = 'attr'):
    ...
    if use ==  'attr':    
        ...
        grass.run_command('v.to.rast',input = input,output = output,use = 'cat',overwrite = True)
### 
...
def grass_raster_r_stream_snap(grass,input,output,stream_rast,accumulation,
                               radius,memory):
    ...
    """      
    grass.run_command('r.stream.snap', input =input,output = output,
                      stream_rast = stream_rast, accumulation = accumulation, 
                      radius = radius, overwrite = True,quiet = 'Ture', 
                      memory = memory)
