...
def FindQ_mean_Da_relaitonship(
    tsubid,
    routing_info,
    Netcat_array,
    SubId_WidDep_array,
    ...
    if WidDep_out_subid > 0:  #### has more than 1 subbains within the domain
        Up_Sub_WidDep = Sub_WidDep_info.loc[
            (Sub_WidDep_info["NEXT_DOWN"] == WidDep_out_subid)
            | (Sub_WidDep_info["HYBAS_ID"] == WidDep_out_subid)
        ]
...
def UpdateChannelinfo(
    ...
    max_manning_n,
    min_manning_n,
    Min_DA_for_Reg_Riv_Slope=100,
):
    routing_info = catinfo[["SubId", "DowSubId"]].astype("float").values

    #########################Define bankfull width and depth
    catinfo_riv = catinfo.loc[catinfo["IsLake"] < 2]
    catinfo_riv_segs = catinfo_riv.loc[
        catinfo_riv["DA"] > Min_DA_for_func_Q_DA * 1000 * 1000
    ]  ## find segment with DA larger than Min_DA_for_func_Q_DA
    #    print("###########################################################################################################################3     ")
    if len(catinfo_riv_segs) <= 1:
        catinfo_riv = catinfo_riv.sort_values(["DA"], ascending=(False))
        tsubid = catinfo_riv["SubId"].values[0]
        k, c, width, depth, qmean, catids = FindQ_mean_Da_relaitonship(
            tsubid, routing_info, Netcat_array, SubId_WidDep_array, WidDep_info
        )
        ...
        if width > 0:
            ...
            catinfo.loc[catinfo["SubId"].isin(catids), "Q_Mean"] = (
                k
                * (
                    catinfo.loc[catinfo["SubId"].isin(catids), "DA"].values
                    / 1000
                    / 1000
                )
                ** c
            )
            catinfo.loc[catinfo["SubId"].isin(catids), "BkfWidth"] = (
                7.2 * catinfo.loc[catinfo["SubId"].isin(catids), "Q_Mean"].values ** 0.5
            )
            catinfo.loc[catinfo["SubId"].isin(catids), "BkfDepth"] = (
                0.27
                * catinfo.loc[catinfo["SubId"].isin(catids), "Q_Mean"].values ** 0.3
            )
    else:
...
def Writecatinfotodbf(catinfo):
    for i in range(0, len(catinfo)):
        ...
        if (
            catinfo["BkfWidth"].values[i] < 0
        ):  #### if no bankfulll width data avaiable for this catchment
            ...
            while twidth < 0 and ccurid > 0:
                downid = catinfo[catinfo["SubId"] == ccurid]["DowSubId"].values[
                    0
                ]  ### get the downstream if of current catchment
                if downid <= 0:  ### if no donwstream catchment exist
                    ...
                    if (
                        ccurid == downid
                    ):  ### if downstream id = current catchment id;; downstream catchemnt is not exist
                        twidth = 1.2345
                        ccurid = -1
                        isdown = -1
                    else:
                        isdown = 1
                        dowcatinfo = catinfo[
                            catinfo["SubId"] == downid
                        ]  ### get downstream id
                        twidth = dowcatinfo["BkfWidth"].values[
                            0
                        ]  ### update twidth catchment with  the downstream id
        ...
        catinfo.loc[i, "Ch_n"] = calculateChannaln(
            catinfo["BkfWidth"].values[i],
            catinfo["BkfDepth"].values[i],
            catinfo["Q_Mean"].values[i],
            catinfo["RivSlope"].values[i],
        )
...
def Generatecatinfo(
    ...
    for i in range(0, len(allcatid)):
        ...
        catrivlen, catrivslp, catrivslp2, rivpath = Getcatrivlenslope_hydroshed(
            rowcol[:, 0],
            rowcol[:, 1],
            rivlen,
            dem,
        ...
        catwidth, catdepth, catQ = Getcatwd(
            catid, finalcat, width, depth, Q_Mean, -1, rivlen
        )  ### width depth in m
        #        arcpy.AddMessage("catid is    " + str(catid) + "    " + str(catwidth))
        catinfo.loc[i, "MeanElev"] = float(
            sum(dem[rowcol[:, 0], rowcol[:, 1]]) / float(len(rowcol))
        )  ### average elevation
        ...
        catinfo.loc[i, "FloodP_n"] = Getfloodplain_n(
            catid, finalcat, rivlen, landuse, landuseinfo
        )
    ...
    for i in range(0, len(Outletinfo)):
        ...
        if obsid > 0:
            ...
            if len(obsinfo.loc[obsinfo["Obs_ID"] == obsid]) > 0:
                catinfo.loc[i, "DA_Obs"] = obsinfo.loc[obsinfo["Obs_ID"] == obsid][
                    "DA_obs"
                ].values[0]
                catinfo.loc[i, "Obs_NM"] = obsinfo.loc[obsinfo["Obs_ID"] == obsid][
                    "STATION_NU"
                ].values[0]
                catinfo.loc[i, "SRC_obs"] = obsinfo.loc[obsinfo["Obs_ID"] == obsid][
                    "SRC_obs"
                ].values[0]
        ...
        floodn = np.unique(
            leninfo.loc[leninfo["Gridcode"] == catid]["mn_average"].values
        )
        if len(rivlen) == 1:
            ...
            if catinfo.loc[i, "IsLake"] != 2:
                print(
                    "Warning  river length of stream  ",
                    catid,
                    "   need check   ",
                    len(rivlen),
                )
...
def Getfloodplain_n(catid, finalcat, rivlen, landuse, landuseinfo):
    ...
    if len(Landtypes) > 0 and float(len(Landtypeid2)) / float(len(Landtypeid1)) >= 0.1:
        ...
        for i in range(0, len(Landtypeid2)):
            iid = Landtypeid2[i]
            sum = sum + landuseinfo[landuseinfo["RasterV"] == iid][
                "MannV"
            ].values * len(np.argwhere(Landtypes == iid))
        ...
        if len(Landtypes2) > 0:
            ...
            for i in range(0, len(Landtypeiicat)):
                ...
                sum = sum + landuseinfo[landuseinfo["RasterV"] == iid][
                    "MannV"
                ].values * len(np.argwhere(Landtypes2 == iid))
