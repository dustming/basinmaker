...
def Getcatrivlenslope_hydroshed(
    catrow,
    catcol,
    rivlen,
    dem,
    ...
    rivout = np.full(
        (len(rivs), 4), -999999999999.000000
    )  ### store riv info for each river path started at boundary of catchment
    rivout2 = np.full(
        (len(rivs), 4), -999999999999.000000
    )  ### store riv info for each river path started at inside of catchment
    for i in range(
        0, len(rivs)
    ):  ## loop for each river segments within the catchment  each river segment was labled with their river legth
        rivsid = rivs[i]
        rivcinfo2 = rivcinfo[
            rivcinfo[:, 0] == rivsid,
        ]  ### get information of the river segment i
        ...
        nout, nearcat = Checkcat2(
            prow, pcol, nrows, ncols, lid, finalcat
        )  #### check if the point (prow,pcol) is close to the catchment boundary, most upstream cell
        ...
        if len(nearcat) > 0:  ### check if one of the near cat is the upstream catchment
            for incat in range(0, len(nearcat)):
                ...
                incat_trow, incat_tcol = Getbasinoutlet(
                    inearcat, finalcat, fac, hydir, nrows, ncols
                )
                incat_nrow, incat_ncol = Nextcell(
                    hydir, incat_trow, incat_tcol
                ...
                if finalcat[incat_nrow, incat_ncol] == lid:
                    iscase1 = 1
        if (
            nout > 0 and iscase1 == 1
        ):  #### this means the strem connect at other catchments. or  this catchment only have one stream some head stream was included
            ...
            while finalcat[nrow, ncol] == finalcat[trow, tcol]:
                ...
                if nrow < 0 or ncol < 0:
                    ...
                    if (
                        nrow > 0 and ncol > 0
                    ):  ##### if they are <0, means the downstream catchment is None, so it is not a error
                        print(
                            "warning : check river system for catchment: ",
                            finalcat[trow, tcol],
                            rivs,
                            icell,
                            len(catrow),
            ...
            if icell > 0:
                ...
                rivout[i, 2] = float(
                    max(abs(rivtemp[0, 1] - rivtemp[icell - 1, 1]), 0.5)
                ) / float(rivtemp[icell - 1, 2])
        ...
        icell = 0
        if (
            len(rivs) > 0 and iscase1 == 0
        ):  # for river segment start inside the catchemnt and do not rieve water from upstream catchment
            ...
            while finalcat[nrow, ncol] == finalcat[trow, tcol]:
                ...
                if flen_orow < 0 or flen_ocol < 0 or icell >= len(rivtemp2):
                    break
                rivpath[nrow, ncol] = 1
                rivtemp2[icell, 0] = rivlen[
                    nrow, ncol
                ]  #### store riv length of each channel cell
                rivtemp2[icell, 1] = dem[
                    nrow, ncol
                ]  #### store riv dem of each channel cell
                rivtemp2[icell, 3] = slope[
                    nrow, ncol
                ]  #### store slope  of each channel cell
                if icell > 0:  ### start from the second cell
                    if (
                        rivtemp2[icell, 0] != rivtemp2[icell - 1, 0]
                    ):  ### come to a new river segment
                        rivtemp2[icell, 2] = (
                            rivtemp2[icell, 0] + rivtemp2[icell - 1, 2]
                        )  ### store cumulated river length of different river segment
                    else:
                        rivtemp2[icell, 2] = rivtemp2[
                            icell - 1, 2
                        ]  ### still in old river segment, do not update cumulated river length
                else:
                    rivtemp2[icell, 2] = rivtemp2[
                        icell, 0
                    ]  ### stroe the river length of the first river segment
                ...
                if nrow < 0 or ncol < 0:  ### if the next cell move out of the domain
                    ...
                    nrow, ncol = -1, -1  ## to break the loop
                if (
                    nrow >= nrows or ncol >= ncols
                ):  ### if the next cell move out of the domain
                    #                    arcpy.AddMessage("out of the boundary")
                    break
            rivtemp2 = rivtemp2[
                rivtemp2[:, 0] > 0,
            ]  ### remove the emmpty positions in the array
            if (
                icell > 0
            ):  ## means there is river grids  icell should equal with number of river grids
                icell = min(icell, len(rivtemp2))
                rivout2[i, 0] = rivtemp2[
                    icell - 1, 2
                ]  ####  get the acuumulated river length for river i
                rivout2[i, 2] = float(
                    max(abs(rivtemp2[0, 1] - rivtemp2[icell - 1, 1]), 0.5)
                ) / float(
                    rivtemp2[icell - 1, 2]
                )  ### (dem_begin - dem_end)/river length, the difference minimum value is 0.5 m
                ...
                if len(rivtemp2) > 0:
                    rivout2[i, 1] = np.mean(
                        rivtemp2[:, 3]
                    )  ####  get the river slope  for river average
                else:
                    rivout2[i, 1] = -9999
    rivout = rivout[
        rivout[:, 0] > 0,
    ...
    if len(rivout) > 0:
        rivout = rivout[
            rivout[:, 0].argsort()
        ]  ### sort with river length of each river segment, the river slopes from longest river segment was used
        outrivlen = rivout[len(rivout) - 1, 0]
        outrivslp = rivout[
            len(rivout) - 1, 2
        ]  ### slope np.mean(slope along the river channel )
        outrivslp2 = rivout[
            len(rivout) - 1, 1
        ]  ### slope max (den_b - dem end , 0.5)/ river length
...
def Getcatrivlenslope_other(
    finalcat, catid, rivlen, dem, fac, hydir, trow, tcol, nrows, ncols, slope, rivpath
):
