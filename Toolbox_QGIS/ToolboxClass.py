...
#            grass.run_command('v.out.ogr', input = 'Hylake',output = os.path.join(self.OutputFolder,'AllLakes.shp'),format= 'ESRI_Shapefile',overwrite = True,quiet = 'Ture') obspoint_snap_r2v
    ...
    def GenerateRavenInput(self,DataFolder = '#', Finalcat_NM = 'finalcat_hru_info',lenThres = 1,iscalmanningn = -1,Startyear = -1,EndYear = -1
        ...
        """
           Funciton that used to generate Ranven inputs. 
           Inputs: 
              DataFolder(string):  folder name that stores a shapefile, the shpfile dbf table includes all required parameters 
              Finalcat_NM(string): The name of the shpfile which includes all required parameters,basially the polygon file generated by the toolbox 
              
           Parameters:
               Parameter needed to define rvh file:
                 lenThres (float):      River length thresthold, river length smaller than this will write as zero in Raven rvh file
                 iscalmanningn(integer): Use manning's coefficient in the shpfile table (>0) or not(<0) a default value 0.035 will provided to all channel (>0)
                 HRU_ID_NM,HRU_Area_NM,Sub_ID_NM(string): Column Names in Finalcat_NM that define the HRU ID, Subbasin ID and HRU AREA
                 Lake_As_Gauge(integer): >0 all lake subbasin will labeled as gauged subbasin, so Raven will export lake balance for this lake, <0 lake subbasin will not labeled as gauge subbasin.
                 
              Paramter needed to define obs.rvt file (Optional):
              
                 Startyear and EndYear(integer):     The year of start and end model simulation, only used to read streamflow observations from US or CA database  
                 WarmUp(integer):                    The warmup length,obs.rvt in warmup period will be -1.2345
                 
              Input needs to copy raven template files(Optional):
                 Template_Folder(string):       A folder contains raven template files includs *.rvi,*.rvp (for paramters, include one line ":RedirectToFile modelchannel.rvp" )    
                 
            Output:
              All output will be stored in DataFolder/Model/RavenInput
              
              Basic outputs: .rvh lake.rvh, modelchannel.rvp 
              if Template_Folder provided, also all template files in the folder
              if  Startyear > 0
                 streamflow observation xxx.rvt for each gauge in the shpfile database will be automatically generagted in DataFolder/Model/RavenInput/obs. 
                 An "obsinfo.csv" information file will also be generated which report the drainage araa difference between observed in shpfile and standard database, and number of missing values for each gauge 
        """
