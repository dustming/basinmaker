...
class LRRT:
    ...
    Path_Sub_Reg_Out_Folder            : integer (optional)
        It is a integer, > 0 indicate the toolbox is working on 
        subregion datasets, < 0 indicate toolbox is not working on 
        subregion dataset.       
    ...
    Generateinputdata
        Preprocessing input data, which will clip, reproject the input 
        datasets. Results are processed input dataset. 
    ...
    AutomatedWatershedsandLakesFilterToolset
        Update the subbasin delineation from "WatershedDiscretizationToolset"
        by adding lake inflow and outflow pourpints. Result from this tool
        is not the final delineation result, because one lake can be divided 
        into several subbasin. 
    RoutingNetworkTopologyUpdateToolset_riv
        ...
        by this tool can be used as inputs for post processing tools
    ...
    Locate_subid_needsbyuser
        ...
        or subbasin ID of the polygon that includes the point in 
        provided point shapefile.
    Select_Routing_product_based_SubId
        Extract region of interest based on provided Subid
    Customize_Routing_Topology
        Simplify the routing product by increase the subbasin drainage area 
    SelectLakes
        Simplify the routing product by removing lakes based on lake area
        or provided lake id list. 
    GenerateHRUS
        Generate HRU polygons and their attributes needed by hydrological
        model. Overlaying subbasin polygon, lake polygon, landuse polygon etc. 
...
########################################################################################
    def Generatmaskregion(self,OutletPoint = '#',Path_Sub_Polygon = '#',Buffer_Distance = 0.0,
        ...
        Notes
        -------
        Outputs are folloing files
                
        self.Path_Maskply      : polygon 
            it is a full path to a polygon file which is the extent of 
            the processing region. 
        MASK                   : raster 
            it is a mask raster stored in grass database,which indicate 
            the processing region. The grass database is located at 
...
##
    def Generateinputdata(self, Is_divid_region = -1):
        ...
        polygon etc with defined processing boundary polygon. The output are 
        prepared input raster files and shapefiles. Raster files will has the 
        ...
        Is_divid_region     : integer 
            ...
            generate divide processing domian into subregions 
...
####################################################################################################3
    ...
    def WatershedDiscretizationToolset(self,accthresold = 100,Is_divid_region = -1,max_memroy = 1024,Search_Radius = 100):
        ...
        Function that used to Generate a subbasin delineation without 
        considering lake. 
        ...
        subbsains. Increasing of accthresold will result in larger 
        subbasins and reduce the number of total subbasins
        Is_divid_region     : integer 
            It is a integer indicate if the tools is called to 
            generate divide processing domian into subregions 
            if it is smaller than 0, means no 
...
############################################################################################
    ...
    max_memroy = 1024):
        ...
        MaximumLakegrids               : integer 
            Maximum lake boundary flow direction grids allow to be 
            corrected. The fow direction of Lakes with the number of lake 
            boundary grids larger than this value will be not corrected.
        Pec_Grid_outlier               : float  
            1- percentage of lake grids that did not flow to lake outlets. 
            Lakes with this percentage larger than this value will be skipped 
            for flow direction correction. 
        Is_divid_region     : integer 
            It is a integer indicate if the tools is called to 
            generate divide processing domian into subregions 
            if it is smaller than 0, means no 
        ...
        SelectedLakes                    : raster 
            it is a raster represent all lakes that are selected by lake
            area threstholds 
        ...
        Select_Connected_lakes           : raster 
            ...
            by lake area threstholds 
        nstr_seg                         : raster  
            it is the river segment for each subbasin              
        Net_cat                          : raster 
            it is a raster represent each subbasin after adding lake inflow 
...
############################################################################3
    def RoutingNetworkTopologyUpdateToolset_riv(self,projection = 'EPSG:3573',max_manning_n = 0.15,min_manning_n = 0.01,Outlet_Obs_ID = -1,Obtain_High_Acc_Cat= -1):
        ...
        ----------
        projection                     : string 
            It is the projected coordinate system, which wiil be used 
            to calcuate area, slope and aspect.
        max_manning_n                  : float 
        ...
        Outlet_Obs_ID                  : integer 
            It is the 'Obs_ID' in the provided observation gauge 
            shapefile. If it is larger than zero. Then only subbasin 
            drainage to this gauge will be generated.
        Obtain_High_Acc_Cat            : integer 
            It is a integer, when it is larger than zero, only subbasins 
            drainage to the outlet with highest flow accumulation value 
            will be generated  
        Notes
        -------
        ...
        or be used as input of other postprocessing tools. All files 
        are stored at a grass database in self.OutputFolder
        
        finalriv_info_ply.shp             : shapefile  
            It is the subbasin polygon before merging lakes catchments and
            need to be processed before used. It is the input for simplify 
            the routing product based on lake area or drianage area.  
...
#        UpdateNonConnectedLakeArea_In_Finalcatinfo(Path_Temp_final_rviply,Non_ConL_cat_info)
    ...
    def Define_Final_Catchment(self,OutputFolder,Path_final_rivply = '#',Path_final_riv = '#'):
        """Define final catchment polygons 

        Function that used to merge catchment polygon covered by 
        the same connected lakes.
        The input catchment polygons before merging for lakes. It 
        is provided with the routing product or is generated by 
        SelectLakes or Select_Routing_product_based_SubId or 
        Customize_Routing_Topology.
        ...
        -------
        This function has no return values, instead will generate following
        files. They are catchment polygons that can be used for hydrological 
        modeling.
