...
Lake1 = arcpy.RasterToNumPyArray(OutputFolder + "/"+'hylake.asc',nodata_to_value=-9999)#np.loadtxt(OutputFolder + "/"+'Lake1.asc',dtype = 'i4',skiprows = 6)
...
alllakeid = np.unique(Lake1)
alllakeid = alllakeid[alllakeid >= 0]

stream = copy.copy(Lake1)
stream[:,:] = -9999

strdemtab = np.full((len(alllakeid),7),-9999) ##BeginLakeid, endlakeid, strid, demmindem, maxdem, begindem, enddem 

arcpy.CreateFeatureclass_management(os.path.dirname(OutputFolder + '/'),os.path.basename("rivnetline_new.shp"), "POLYLINE")
dbfile = OutputFolder + '/' + "rivnetline_new.shp"
inFeatures = dbfile
fieldPrecision = 10
field_scale = 3
arcpy.AddField_management(dbfile, "LakeId", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "DownLakeId", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "Beginelve", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "Endelve", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "Maxelve", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "Minelve", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "Rivlen", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "Ngrids", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")

cursor = arcpy.da.InsertCursor(OutputFolder + '/'+ "rivnetline_new.shp", ["SHAPE@","LakeId","DownLakeId","Beginelve","Endelve","Maxelve","Minelve","Rivlen","Ngrids"])  # arcpy.InsertCursor(OutputFolder + '/'+ "rivnetline.shp")
...
##find lake outlet 
    olakerow,olakecol = Getbasinoutlet(lakeid,Lake1,fac,hydir,nrows,ncols)

    nrow, ncol = olakerow,olakecol  ### current location 
    

    dembegin = hydem[nrow,ncol] ### store elevation at the beginning of the river network
    eleva = np.full((1000000,1),-9999)
    k = 0
    while Lake1[nrow,ncol] == lakeid or Lake1[nrow,ncol] <= 0:
    
#        point.X = (ncol+0.5) * cellSize + (-95.635104232996)
#        point.Y = (nrows - nrow - 0.5) * cellSize + 47.39252745079
        array.add(arcpy.Point((ncol+0.5) * cellSize + (-95.635104232996), (nrows - nrow - 0.5) * cellSize + 47.39252745079)) 
         
        stream[nrow,ncol] = lakeid   ### store river network between two lakes
        if hydem[nrow,ncol] > 0:
            demend = hydem[nrow,ncol]
        eleva[k] = hydem[nrow,ncol]
        
        nextrow,nextcol = Nextcell(hydir,nrow,ncol)
        
        
        if nextrow < 0 or nextcol < 0:
            break
        if nextrow >= nrows or nextcol >= ncols:
            break
        
        nrow = nextrow
        ncol = nextcol
        
        strdemtab[i,1] = Lake1[nrow,ncol]  ### downstram lake id
        k = k + 1
        #print(nrow,ncol,Lake1[nrow,ncol])

        
   ### store the evelevation at the end of the river network 
    eleva = eleva[eleva >= 0]
    strdemtab[i,0] = lakeid
    
    strdemtab[i,2] = lakeid
    strdemtab[i,3] = dembegin
    strdemtab[i,4] = demend
    if(len(eleva) > 0):
        strdemtab[i,5] = max(eleva)
        strdemtab[i,6] = min(eleva)
#["SHAPE@","LakeId","DownLakeId","Beginelve","Endelve","Maxelve","Minelve","Rivlen"]        
    cursor.insertRow([arcpy.Polyline(array),lakeid,strdemtab[i,1],strdemtab[i,3],strdemtab[i,4],strdemtab[i,5],strdemtab[i,6],-99.999,k])     
    array.removeAll()

del cursor

#arcpy.CopyFeatures_management(features, OutputFolder + '/'+ "rivnetline.shp")
np.savetxt(OutputFolder+"/"+"streleva_betweenlakes.csv",strdemtab,delimiter=",")
writeraster(OutputFolder+"/"+"rivnet.asc",stream,OutputFolder + "/"+'dir.asc')