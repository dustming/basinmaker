...
def Getcatrivlenslope(catrow,catcol,rivlen,dem,fac,hydir,finalcat,trow,tcol,nrows,ncols,slope):
    ...
    for i in range(0,len(rivs)):
        ...
        if nout > 0:
            ...
            if icell > 0:
#                print len(rivtemp),icell,finalcat[prow,pcol],rivs
...
catinfo = np.full((len(allcatid),40),-99999999999.00)
catinfo = Generatecatinfo(finalcat,fac,hydir,Lake1,hydem,area,cat,allsubinfo,catinfo,allcatid,allLakinfo,width,depth,rivlen,obs,nrows,ncols,slope)
np.savetxt(OutputFolder+"/"+"catinfo.csv",catinfo,delimiter=",")
...
Writecatinfotodbf(OutputFolder + "/",catinfo)
