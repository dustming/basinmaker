...
for i in range(0,len(list)):
	...
	if os.path.isfile(in_outputf2) and not os.path.isfile(ot_check):
		...
		k = k + 1
dbftocsv(tarshp +"/"+ "finalcat_info.dbf",tarshp +"/"+ "finalcat_info.csv")
dbfile = tarshp +"/"+ "finalcat_info.shp"
inFeatures = dbfile
fieldPrecision = 10
field_scale = 10
arcpy.AddField_management(dbfile, "NSUBID", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
arcpy.AddField_management(dbfile, "NDOWSUBID", "FLOAT", fieldPrecision,field_scale,"", "", "NULLABLE","","")
...
for row in rows:
	if row.BASINID == obasinid:
		row.NSUBID = row.SUBID + baseid
		if row.DOWSUBID > 0:
			row.NDOWSUBID = row.DOWSUBID + baseid
		else:
			row.NDOWSUBID = -1
		currcatid = row.SUBID + baseid
	else:
		baseid = currcatid + 2
		row.NSUBID =  row.SUBID + baseid
		if row.DOWSUBID > 0:
