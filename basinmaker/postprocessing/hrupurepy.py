...
#    lakehruinfo_landhrus.to_file(path_to_landhru_shp)
    ...
    if Path_Other_Ply_1 != "#":
        ...
        if Other_Ply_ID_1 not in land_o1_clean.columns:
            print("Other_Ply_1 polygon attribute table do not contain: ",
                  Other_Ply_ID_1)
            sys.exit()
    ...
    if Path_Landuse_Ply == "#" and Path_Soil_Ply == "#" and Path_Veg_Ply == "#" and Path_Other_Ply_1 == "#" and Path_Other_Ply_2 == "#":

        path_to_hru_temp_shp = RasterHRUUnionInt64_Routing(OutputFolder, tempfolder, Merge_layer_shp_list,
        ...
        HRU_temp1 = decode_hru_attri_ids_int32_routing(HRU_temp1, lakehruinfo, Landuse_ID, Soil_ID,
    ...
    HRU_draf_final = clean_attribute_name_purepy(
        HRU_draf_final, COLUMN_NAMES_CONSTANT_HRU)
...
#    HRU_draf_final_wgs_84 = HRU_draf_final_wgs_84[['HRU_ID','geometry']]
    ...
    for TOLERANCE in TOLERANCEs:
        HRU_draf_final_wgs_84['geometry'] = HRU_draf_final_wgs_84.simplify(
            TOLERANCE)
...
#    arcpy.RepairGeometry_management(Path_Subbasin_ply)
    ...
    if Path_Connect_Lake_ply == "#" and Path_Non_Connect_Lake_ply == "#":
        ...
        cat_info.to_file(os.path.join(
            OutputFolder, 'finalcat_hru_lake_info.shp'))
...
#    arcpy.RepairGeometry_management(os.path.join(tempfolder,'cat_ply.shp'))
    ...
    inFeatures = [[Path_Subbasin_ply, 1], [
...
#    arcpy.RepairGeometry_management(os.path.join(tempfolder,'cat_lake_union.shp'))
    ...
    sub_lake_info = sub_lake_info.sort_values(
        by=['SubId', Lake_Id]).copy(deep=True).reset_index()
...
def Define_HRU_Attributes_purepy(
    ...
    hruinfo_area = hruinfo_area.sort_values(
        by=[Sub_ID, Soil_ID, Landuse_ID]).copy(deep=True).reset_index()
    ...
    hruinfo_area_update_attribute = clean_geometry_purepy(
        hruinfo_area_update_attribute, set_precision=1)
...
#    print(len(hruinfo_area_update_attribute),len(hruinfo_new))
    ...
    hruinfo_simple = add_centroid_in_wgs84(
        hruinfo_simple, "HRU_CenX", "HRU_CenY")
...
#        proj_clip_raster(os.path.join(tempfolder,"demproj.tif"),os.path.join(tempfolder,"demclip.tif"),prj_crs,os.path.join(tempfolder,"hru_proj.shp"))
        gdal_slope_raster(os.path.join(tempfolder, "demproj.tif"),
                          os.path.join(tempfolder, "demslope.tif"))
        gdal_aspect_raster(os.path.join(tempfolder, "demproj.tif"),
                           os.path.join(tempfolder, "demaspect.tif"))

        table_elv = ZonalStats(hru_proj, os.path.join(
            tempfolder, "demproj.tif"), 'mean', 'HRU_ID_New', 'MeanElev')
        table_asp = ZonalStats(hru_proj, os.path.join(
            tempfolder, "demaspect.tif"), 'mean', 'HRU_ID_New', 'BasAspect')
        table_slp = ZonalStats(hru_proj, os.path.join(
            tempfolder, "demslope.tif"), 'mean', 'HRU_ID_New', 'BasSlope')
        ...
        hruinfo_add_slp_asp = hruinfo_add_slp_asp.sort_values(
            by=[Sub_ID, Soil_ID, Landuse_ID]).copy(deep=True).reset_index()
        hruinfo_add_slp_asp['HRU_ID'] = hruinfo_add_slp_asp.index + 1
#        print(len(hruinfo_add_slp_asp))
    else:

        hruinfo_add_slp_asp = hruinfo_simple.sort_values(
            by=[Sub_ID, Soil_ID, Landuse_ID]).copy(deep=True).reset_index()
        hruinfo_add_slp_asp['HRU_ID'] = hruinfo_add_slp_asp.index + 1
    ...
    hruinfo_add_slp_asp = adjust_HRUs_area_based_on_ply_sub_area(
        hruinfo_add_slp_asp)

    hruinfo_add_slp_asp = clean_geometry_purepy(
        hruinfo_add_slp_asp, set_precision=1)
#    print(len(hruinfo_add_slp_asp))
