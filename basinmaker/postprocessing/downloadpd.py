    ...
    if product_name == 'NALRP':
        ...
        if gauge_name != '#':
            gauge_info = pd.read_csv(
                "https://github.com/dustming/RoutingTool/wiki/Files/obs_gauges_NA_v2-1.csv")
            gauge_info_sl = gauge_info[gauge_info['Obs_NM'] == gauge_name]
            if len(gauge_info_sl) < 1:
                print("The gauge ", gauge_name,
                      "is not included in the routing product ")
                SubId = -1
                product_path = '#'
            else:
                if gauge_info_sl['Use_region'].values[0] < 1:
                    ...
                    os.system('unzip drainage_region_%s_%s_%s.zip' % (
                        str(region_id).zfill(4), str(subreg_id).zfill(5), version))
...
#                    subreg_id = int(gauge_info_sl['Sub_Reg'].values[0])
                    ...
                    os.system('unzip drainage_region_%s_v2-1.zip' %
                              (str(region_id).zfill(4)))
                    SubId = gauge_info_sl['SubId'].values[0]
                    product_name = "drainage_region_%s_v2-1" % (
                        str(region_id).zfill(4))
                    product_path = os.path.join(os.getcwd(), product_name)
                    print("The needed product locates at:", product_path)
                    print("The Subbasin Id of the interested gauge is:", SubId)
            ...
            if len(gauge_info_sl) < 1 or gauge_info_sl['SubId'].values[0] < 0:
                ...
                if gauge_info_sl['Use_region'].values[0] < 1:
                    ...
                    url = "https://drive.google.com/u/0/uc?id=%s&export=download" % (
                        url_veiw[5])
                    output = 'drainage_region_%s_%s_%s.zip' % (
