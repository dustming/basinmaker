...
def combine_catchments_covered_by_the_same_lake_purepy(
    ...
    Path_Temp_final_rviply = os.path.join(
        OutputFolder, "temp_finalriv_ply" + str(np.random.randint(1, 10000 + 1)) + ".shp")

    Path_Temp_final_rvi = os.path.join(
        OutputFolder, "temp_finalriv_riv" + str(np.random.randint(1, 10000 + 1)) + ".shp")
...
#    finalrivply_info['DrainArea'] = finalrivply_info['DrainArea'].astype('float')
    ...
    mapoldnew_info = remove_possible_small_subbasins(
        mapoldnew_info=mapoldnew_info, area_thresthold=area_thresthold, length_thresthold=length_thresthold)
    ...
    if len(os.path.basename(Path_Catchment_Polygon).split('_')) == 5:
        cat_name = "finalcat_info_" + \
            os.path.basename(Path_Catchment_Polygon).split('_')[4]
        riv_name = "finalcat_info_riv_" + \
            os.path.basename(Path_Catchment_Polygon).split('_')[4]
    else:
