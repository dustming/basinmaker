...
#    HRU_temp1['HRU_ID_str'] = HRU_temp1['HRU_ID_T'].str.zfill(10)
    ...
    HRU_temp1 = pd.merge(HRU_temp1, lakehruinfo_id,
                         how='inner', on='HRULake_ID')
    ...
    HRU_temp1 = HRU_temp1.loc[(HRU_temp1['HRULake_ID'] > 0)
                              | (HRU_temp1['SubId'] > 0)]
...
def RasterHRUUnionInt32(OutputFolder, tempfolder, Merge_layer_shp_list,
 ...
 #    'o1':              1,
    ...
    raster_par_list = [x_min, x_max, y_min,
                       y_max, x_res, pixel_size, y_res, ras_crs]

    # rasterize
    vector_to_raster(path_to_sub_lake_raster, Sub_Lake_HRU_Layer,
                     'HRULake_ID', raster_par_list, touch='False')
    ...
    for i in range(0, len(Merge_layer_shp_list)):
        ...
        out_raster = os.path.join(
            tempfolder, "%s_ras_layer.tif" % (ID_shpfile))
        vector_to_raster(out_raster, merge_shpfile, ID_shpfile,
                         raster_par_list, touch='False')

        raster_layer_i = gdal.Open(out_raster)
        raster_layer_i_a = np.array(
            raster_layer_i.GetRasterBand(1).ReadAsArray())
        if raster_layer_i_a.shape[0] == HRU_shape[0] and raster_layer_i_a.shape[1] == HRU_shape[1]:
            ...
            print(ID_shpfile, "polygon may not fully cover the subbasin domain ",
                  land_raster_a.shape)
            sys.exit()

    HRU_a[HRU_a < raster_value_multi[Sub_ID]] = -9999
    array_to_raster(path_to_hru_temp_raster, HRU_a,
                    raster_par_list, touch="False", type=gdal.GDT_Int32)

    raster_to_vector(path_to_hru_temp_shp,
                     path_to_hru_temp_raster, raster_par_list)
...
def RasterHRUUnionInt64_Routing(OutputFolder, tempfolder, Merge_layer_shp_list,
                                Merge_ID_list, Sub_Lake_HRU_Layer, Sub_ID,
                                Landuse_ID, Soil_ID, Veg_ID, Other_Ply_ID_1,
                                Other_Ply_ID_2, pixel_size, lakehruinfo):
...
def decode_hru_attri_ids_int32_routing(HRU_temp1, lakehruinfo, Landuse_ID, Soil_ID, Veg_ID, Other_Ply_ID_1, Other_Ply_ID_2):
    #     100000000000000,
    #          0007422000,
    #              2147483647,
    #    'sub':            10,
    ...
    HRU_temp1['HRU_ID_str'] = HRU_temp1['HRU_ID_T'].astype('int64').astype(str)
#    HRU_temp1['HRU_ID_str'] = HRU_temp1['HRU_ID_T'].str.zfill(10)
    HRU_temp1['HRULake_ID'] = HRU_temp1['HRU_ID_str'].str[:-1].astype(int)
    HRU_temp1 = pd.merge(HRU_temp1, lakehruinfo_id,
                         how='inner', on='HRULake_ID')
    HRU_temp1[Landuse_ID] = HRU_temp1['HRU_ID_str'].str[-1:].astype(int)
    HRU_temp1[Veg_ID] = HRU_temp1['HRU_ID_str'].str[-1:].astype(int)
    HRU_temp1[Soil_ID] = HRU_temp1['HRU_ID_str'].str[-1:].astype(int)
    HRU_temp1[Other_Ply_ID_1] = HRU_temp1['HRU_ID_str'].str[-1:].astype(int)
    HRU_temp1[Other_Ply_ID_2] = HRU_temp1['HRU_ID_str'].str[-1:].astype(int)
    HRU_temp1 = HRU_temp1.loc[(HRU_temp1['HRULake_ID'] > 0)
                              | (HRU_temp1['SubId'] > 0)]
